...
```{r}
model_nuts2 <- plm(gini ~ vekst, data = pdata, model = "within", effect = "individual")
summary(model_nuts2)

```

I denne modellen beregnes effekten av BNPPI på GINI kontrollert for tids invariante tidsforskjellem mellom NUTS2 regionene. BNPPI estimatet viser en negativ korelasjon på -0,022. Dette indikerer at når vi analyserer effekten innenfor samme regioner over tid vil øknomisk vekst føre til reduksjon i ulikhet. R2 viser at kun ca 1,1% av GINI variasjonene forventes å kunne forklare med endring i BNPPI, 



```{r}
model_aar <- plm(gini ~ vekst, data = pdata, model = "within", effect = "time")
summary(model_aar)
```

```{r}
model_tw <- plm(gini ~ vekst, data = pdata, model = "within", effect = "twoways")
summary(model_tw)
```
...
model_land <- plm(gini ~ vekst + factor(land), data = pdata, model = "within", effect = "individual")
summary(model_land)
```
