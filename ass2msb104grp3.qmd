...
```{r}
#Lager et samlet datasett med alle variabler som skal analysers
...
Sammenstilt <- Sammenstilt %>%
  mutate(
    ...
    bpk = as.numeric(bpk),
    hoyere_utdanning = as.numeric(hoyere_utdanning)
  )

# Estimer lineær regresjon
model_multi <- lm(GINI_BNPPI ~ BNPPI_vekst + arbeidsledighet + bpk + hoyere_utdanning, data = Sammenstilt)

# Hent ut og formater resultater
regression_results <- tidy(model_multi) %>%
  mutate(
    Estimate = format(estimate, digits = 4, scientific = FALSE),
    `Std. Error` = format(std.error, digits = 4, scientific = FALSE),
    `t value` = format(statistic, digits = 4, scientific = FALSE),
    `Pr(>|t|)` = format(p.value, digits = 4, scientific = FALSE)
  ) %>%
  select(term, Estimate, `Std. Error`, `t value`, `Pr(>|t|)`)

# Vis tabellen
kable(regression_results, caption = "Regresjonsresultater")
```
...
cat("F-statistic: ", format(round(f_stat, 2), nsmall = 2), " on ", f_numdf, " and ", f_dendf, " DF\n")
