...
# Sørg for at du har en variabel med landkode (to første bokstaver i region)
Kombinert <- Kombinert %>%
  mutate(land = substr(geo, 1, 2))

# Beregn gjennomsnittlig BNP per innbygger per land
SnittLand <- Kombinert %>%
  group_by(land) %>%
  summarise(gjennomsnitt_bnp = mean(bnp_per_innbygger, na.rm = TRUE))

# Plot: én stolpe per land
ggplot(SnittLand, aes(x = land, y = gjennomsnitt_bnp, fill = land)) +
...
ggplot(SnittTidLand, aes(x = aar, y = gjennomsnitt_bnp, color = land_navn)) +
  ...
  scale_x_continuous(
    breaks = seq(2000, 2023, 3)         # litt ekstra plass til etiketter
  ) +
