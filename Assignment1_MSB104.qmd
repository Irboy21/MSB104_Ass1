...
# define not in
`%nin%` <- Negate(`%in%`)
# Virker ikke på mac, kanskje heller ikke på andre unix
if(.Platform$OS.type %nin% c("unix")) {
...
saveRDS(Populasjon, "data/Populasjon.rds")
saveRDS(BNP, "data/BNP.rds")
```

```{r}
# Henter inn regional BNP datasett fra excell
bnp_wide<- read_excel("GDP_Ass1.xlsx", sheet = 2, col_types = "text", na = c(NA, ":")) %>%
  clean_names()
```
...
saveRDS(PopulasjonLang, "data/PopulasjonLang.rds")
saveRDS(BNPLang, "data/BNPLang.rds")
```
...
Hver observasjon identifiseres ved regionkode(**geo**) og år (**time**), samt kjønn (**sex**) og alder (**age**).
I denne analysen benyttes kun total befolkning (**sex = T**,**age = TOTAL**), slik at dataene ikke er splittet etter kjønn eller alder.

Det finnes ulike hovedmetoder å beregne brutto-nasjonaltprodukt.
Eurostat har valgt å benytte den såkalte «inntekstmetoden».
Eurostat velger denne metoden over utgiftsmetoden grunnet mangel på data for gode-overføringer mellom regioner.
...
bnp_pop <- pop %>% 
left_join(bnp, by = join_by(region, aar)) |> 
mutate(
  bnp_pi = (bnp * 1e6) / pop
  )
...
saveRDS(Kombinert, "data/Kombinert.rds")
```

```{r}
# Fjerner alle regioner med NA verdier
# Trenger dere denne. Kanskje like greit å bare filtrere ut NA når nødvendig?
...
saveRDS(KombinertRen, "data/KombinertRen.rds")
```

```{r}
n_total <- nrow(bnp_pop)
...
bnp_pop %>%
  summarise(
    across(.cols = everything(), ~ sum(is.na(.x)))
    ) %>%
  mutate(n = n_total) %>%
  select(n, everything()) %>%
  kable(
...
bnp_pop %>%
  vtable::st(
    summ = c('notNA(x)', 'countNA(x)', 'mean(x)','sd(x)','min(x)','max(x)'),
    out = "return"
  ) |> 
  mutate(
    across(
...
Vi har da en tabell som viser BNP per innbygger i hvert av NUTS3 områdene fordelt på år.
I hver observasjon hvor det forekommer en NA verdi i populasjon og eller total BNP vil vi også få at BNPPI har verdien NA.
...
```{r}
bnp_pop %>%
  filter(complete.cases(pop, bnp, bnp_pi)) |> 
  mutate(
    aar = as.numeric(aar),
    bnp_pi = as.numeric(bnp_pi)
  ) %>% 
  group_by(aar) %>%  
  summarise(
    gjennomsnitt_bnp_pi = mean(bnp_pi),
    median_bnp_pi       = median(bnp_pi),
    minste_bnp_pi       = min(bnp_pi),
    største_bnp_pi      = max(bnp_pi)
  ) %>%
  arrange(aar) %>%
  mutate(across(
    gjennomsnitt_bnp_pi:største_bnp_pi,
...
```{r}
#| label: tbl-desc-bnp-pop
#| tbl-cap: Deskreptiv statistikk for populasjon og BNP. Utvikling i BNP per innbygger (gjennomsnitt, median, minimum og maksimum) per år.
bnp_pop %>%
  mutate(
    # bedre å gjøre denne om til date object
    aar = ymd(paste(aar, "01-01", sep = "-"))
  ) %>% 
  group_by(aar) %>%  
  summarise(
    gjennomsnitt_bnp_pi = mean(bnp_pi, na.rm = TRUE),
    median_bnp_pi       = median(bnp_pi, na.rm = TRUE),
    minste_bnp_pi       = min(bnp_pi, na.rm = TRUE),
    største_bnp_pi      = max(bnp_pi, na.rm = TRUE)
  ) %>%
  arrange(aar) %>%
  # Velger å heller gjøre dette i flextable
  # mutate(across(
  #   gjennomsnitt_bnp_pi:største_bnp_pi,
  #   ~ label_number(big.mark = " ")(.)
  # )) %>%
rename(
  `År` = aar, 
  `Gjennomsnitt\n BNPPI` = gjennomsnitt_bnp_pi, 
  `Median\n BNPPI` = median_bnp_pi, 
  `Minste\n BNPPI` = minste_bnp_pi, 
  `Største\n BNPPI`  = største_bnp_pi
) |> 
as_flextable(
  show_coltype = FALSE,
  max_row = 30
...
I @tbl-desc-bnp-pop ser vi utviklingen av gjennomsnitt, median, minste og høyeste verdi for BNP per inbygger for fra år 2000 til år 2023.
...
For å enklere visualisere dette lager vi linjediagrammer for sentrale mål.
...
bnp_pop %>%
  group_by(aar) %>% 
  summarise(
    Gjennomsnitt = mean(bnp_pi, na.rm = TRUE),
    Median = median(bnp_pi, na.rm = TRUE)
  ) %>%
  pivot_longer(
...
SnittTidLand <- bnp_pop %>%
  group_by(land, aar) %>%
  summarise(
    gjennomsnitt_bnp = mean(bnp_pi, na.rm = TRUE),
    .groups = "drop"
  )
...
saveRDS(SnittTidLand, "data/SnittTidLand.rds")
...
```{r}
#| warning: false
#| label: fig-change-bnp-pc
#| tbl-cap: Utvikling i gjennomsnittlig BNP per innbygger (2000–2023).
bnp_pop %>%
  mutate(
    land = str_sub(geo, 1, 2), 
    land_navn = case_when(
      land == "IT" ~ "Italia",
      land == "FI" ~ "Finland", 
      land == "LT" ~ "Litauen",
  ...
  group_by(land, år, land_navn) %>%
  summarise(
    `bnp\n per capita` = mean(bnp_pi, na.rm = TRUE),
    .groups = "drop"
  ) |> 
  ggplot(
    # For å få ok benevnelse på aksene
    mapping = aes(x = år, y = `bnp\n per capita`)
  ) + 
  geom_line(
    mapping = aes(group = land, colour = land),
    linewidth = 1
  ) +
  geom_text_repel(
    data = . %>% filter(år == 2022),
    aes(label = land_navn, colour = land),
    nudge_x = 2.5,
    size = 3,
    direction = "y",
    hjust = 0,
...
Tankegangen er at aktiviteter som koster penger å gjennomføre på kvelden som regel trenger lys, og jo rikere et land/ region er jo flere har råd til å delta i slike aktiviteter.
Ved å analysere satellittbilder på nattestid kan mengden lys være en indikator på velstanden i området.

Nytteverdien fra denne vinklingen kommer av at det er stor variasjon på hvor godt dokumentert regionale ulikheter er fra land til land.
Artikkelen påpeker at tidligere studier på området fokuserer på velutviklete nasjoner, da det ikke finnes tilgjengelige gode nok data for mindre utviklete land på regionalt nivå.
Dersom det kan vises at dette konseptet holder vann får forskere enn ny måte å analysere mindre velstående land sine regionale ulikhet.
...
# TRor vi greier oss fint uten ren versjonen også her
bnp_pop<- bnp_pop %>%
  mutate(
    nuts2 = str_sub(geo, 1, 4),
    geo = str_trim(str_to_upper(geo))
  )
...
saveRDS(NUTS2_data, "data/NUTS2_data.rds")
```
...
```{r}
Gini_IT <- bnp_pop %>%
  filter(land == "IT", !is.na(bnp_pi), bnp_pi > 0) %>%
  group_by(nuts2, aar) %>%
  summarise(
    gini = round(gini.wtd(
      x = bnp_pi,
...
ggplot(
  ...
  data = Gini_IT |> filter(gini > 0), 
  mapping = aes(x = aar, y = gini, color = nuts2, group = nuts2)) +
  geom_line(linewidth = 1.2) +
  labs(
...
saveRDS(Gini_IT, "data/Gini_IT.rds")
```
...
```{r}
Gini_FI <- bnp_pop %>%
  filter(land == "FI", !is.na(bnp_pi), bnp_pi > 0) %>%
  group_by(nuts2, aar) %>%
  summarise(
    gini = round(gini.wtd(
      x = bnp_pi,
...
ggplot(
  data = Gini_FI |> filter(gini > 0), 
  aes(x = aar, y = gini, color = nuts2, group = nuts2)) +
  geom_line(linewidth = 1.2) +
  labs(
...
saveRDS(Gini_FI, "data/Gini_FI.rds")
...
```{r}
Gini_RO <- bnp_pop %>%
  filter(land == "RO", !is.na(bnp_pi), bnp_pi > 0) %>%
  group_by(nuts2, aar,) %>%
  summarise(
    gini = round(gini.wtd(
      x = bnp_pi,
...
saveRDS(Gini_RO, "data/Gini_RO.rds")
...
```{r}
Gini_LT <-bnp_pop %>%
  filter(land == "LT", !is.na(bnp_pi), bnp_pi > 0) %>%
  group_by(nuts2, aar,) %>%
  summarise(
    gini = round(gini.wtd(
      x = bnp_pi,
...
saveRDS(Gini_LT, "data/Gini_LT.rds")
```
...
ggplot(
  Gini_alle |> filter(gini > 0), 
  mapping = aes(x = aar, y = gini, color = land, group = nuts2)) +
  geom_line(linewidth = 1.2) +
    scale_color_manual(
    values = c(
      "Italia" = "#CCCC00",    
...
saveRDS(Gini_alle, "data/Gini_alle.rds")
```
...
saveRDS(Gini_gjennomsnitt, "data/Gini_gjennomsnitt.rds")
```
...
saveRDS(Gini_total, "data/Gini_total.rds")
```
...
saveRDS(NUTS1_data, "data/NUTS1_data.rds")
```

```{r}
Gini_FI_NUTS1 <- bnp_pop %>%
  filter(land == "FI", !is.na(bnp_pi), bnp_pi > 0) %>%
  group_by(nuts1, aar) %>%
  summarise(
    gini = round(gini.wtd(
...
saveRDS(Gini_FI_NUTS1, "data/Gini_FI_NUTS1.rds")
saveRDS(Gini_IT_NUTS1, "data/Gini_IT_NUTS1.rds")
saveRDS(Gini_LT_NUTS1, "data/Gini_LT_NUTS1.rds")
saveRDS(Gini_RO_NUTS1, "data/Gini_RO_NUTS1.rds")
saveRDS(Gini_alle_NUTS1, "data/Gini_FI_NUTS1.rds")
```

Her er det laget et linjediagram som viser utviklingen av GINI koeffisient på NUTS1-nivå (Land) basert på fordelingen av BNPPI i NUTS2 regionene.
Koeffisientene sier noe om fordelingen av BNPPI mellom NUTS2 regioner sett opp mot hele landet.
Vi ser at Italia sin koeffisienten ligger mellom 0,17 og 0,19.
Dette indikerer at det i Italia er en større skjevfordeling mellom NUTS2 regionene, enn det er i NUTS3 regionene satt opp mot regioner i samme NUTS2.
...
Samlet sett viser tidsserien for Gini-koeffisientene på NUTS2-nivå at de fleste landene opplever svak økning i ulikhet etter 2010, noe som sammenfaller med finanskrisen og senere perioder med økonomisk omstilling.
Øst-Europa (spesielt Romania og Litauen) viser tegn til økonomisk konvergens på nasjonalt nivå, men ikke nødvendigvis på regionalt nivå – de rikeste regionene vokser fortsatt raskere enn resten.

Når det gjelder regionale ulikheter, indikerer resultatene at økonomisk vekst i EU ikke har vært likt fordelt.
Land som Finland har klart å opprettholde en jevn fordeling, mens andre land, særlig Romania, som vi ser har en store variasjoner i hvor jevnt fordelt BNPPI-er fra region til region.

## Del C: Dokumentasjon av AI

Vi har benyttet chatGTP modell 4.1 og 5 gjennom denne oppgaven.
Først har vi benyttet AI til å tolke og forstå oppgaveteksten samt gitt oss et utgangspunkt til hvor vi skal starte med behandlingen av eurostat dataene.
Videre har vi benyttet AI til å finne/løse feil i kodene.
Ledeteksten vi har benyttet er å gi oppgaveteksten og pensum til chatGPT.
Dette ga oss flere ideer om ulike modeller som vi har benyttet i denne oppgaven.
