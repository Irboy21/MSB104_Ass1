...
# Beregn Gini
gini_total <- Gini(Kombinert$bnp_per_innbygger, na.rm = TRUE)

# Per land
GiniLand <- Kombinert %>%
  group_by(land) %>%
  summarise(Gini = Gini(bnp_per_innbygger, na.rm = TRUE))
...
ggplot(GiniLand, aes(x = land_navn, y = Gini, fill = land_navn)) +
  geom_col() +
  labs(
    title = "Gini-koeffisient for BNP per innbygger per land",
    x = "Land",
    y = "Gini-koeffisient"
  ) +
  theme_minimal() +
